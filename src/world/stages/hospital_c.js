...
export const meta = {
   ...
   *
   * **처음에 0.10 으로 넣었다가 화면에서 아무 차이가 안 났다** — 먹통 52.7% → 48.3%,
   * 평균 휘도 +4%. B1 의 `litAmbient: 1.15` 주석에 "0.4 대로는 켜졌다가 안 읽힌다"고
   * 이미 적혀 있었는데 그걸 놓쳤다. 3지점 실측으로 다시 고른 값이다:
   *
   *   앰비언트   0.035(꺼짐) → 0.25 → 0.45 → **0.70** → 1.15(B1급)
   *   먹통 %       52.6        46.2   43.2    39.6      33.3
   *
   * 0.70 이면 어두운 픽셀의 4분의 1이 사라져 "전기가 왔다"가 읽히는데,
   * B1급(1.15)으로 올려도 2F 는 여전히 33% 가 먹통이라 **손전등은 계속 필요하다.**
   * 어둠을 잃지 않는 선에서 가장 크게 읽히는 지점이라 여기로 정했다.
   */
  poweredMood: { ambientIntensity: 0.70, fogDensity: 0.048 },
