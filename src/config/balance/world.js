...
export const EVENTS = {
  ...
  roof: {
    ...
    /**
     * ── 피날레 연출 (2026-08-09 추가) ────────────────────────────────
     * 그전까지 이 마지막 장면은 **게임에서 가장 조용한 순간**이었다.
     * 신호탄은 손전등 클릭음(`flashlight`)을 돌려 쓰고 있었고, 헬기는 소리
     * 파일이 아예 없어서 90초가 지나면 조명만 바뀌고 끝났다.
     */
    boomDelay: 0.9,        // 쏜 뒤 머리 위에서 터지기까지(초). 0 이면 "쏘아 올렸다"가 안 읽힌다
    launchShake: 0.25,     // 발사 반동. 총(0.40)보다 약하다 — 위로 쏘는 것이라
    boomShake: 0.85,       // 터질 때. SHAKE 는 제곱해서 쓰므로 0.85 면 확 온다
    boomVolume: 0.9,       // 파형 최대 0.699 라 1.0 이어도 안 깨지지만 여유를 둔다
    /**
     * 터진 신호탄이 옥상을 물들이는 시간(초)과 색.
     * 달빛 청회색(0x8fa6c4) 위에 주황이 얹혀서 **머리 위에 뭔가 타고 있다**가 읽힌다.
     * 끝나면 meta.mood 로 정확히 되돌린다 — 안 되돌리면 다음 판까지 물든 채로 남는다.
     *
     * **포그 색은 건드리지 않는다.** 처음엔 0x53384a 를 같이 넘겼는데, 포그 색은
     * `scene.background` 이기도 해서 **하늘 전체가 자홍색이 됐다** — 조명탄이 아니라
     * 렌더링이 깨진 것처럼 보였다 (scratchpad/f2_신호탄.png 로 확인).
     * 신호탄은 바닥과 물건을 비추는 것이지 하늘을 칠하는 것이 아니다.
     */
    flareTintSeconds: 14,
    flareTint: { ambientColor: 0xc9834e, ambientIntensity: 0.78 },

    /**
     * 헬기 소리는 **도착 한참 전부터** 들린다. 멀리서 로터 소리가 커지는 것이
     * "조금만 더"를 만든다. 도착음만 있으면 90초가 그냥 견디는 시간이다.
     * 남은 시간(초) 기준 — [45, 25, 10] 이면 45초 전에 처음 들린다.
     */
    heliCueAt: [45, 25, 10],
    heliCueVolume: [0.5, 0.75, 1.0],   // 위와 같은 길이. 점점 커진다
    heliArriveLead: 3,     // 도착 몇 초 전에 로터 소리가 덮치는가
    heliArriveVolume: 1.0,
    heliRepeatEvery: 7,    // 도착 후 로터 소리를 다시 트는 간격(초). 파일이 8초다
    heliRepeats: 4,        // 몇 번 더. 난간까지 뛰어가는 동안 계속 들려야 한다
    /**
     * 헬기가 오면 조명이 바뀐다. **예전에는 여기서 화면이 더 어두워졌다** —
     * `setMood({ ambientIntensity: 0.4, fogDensity: 0.018 })` 만 넘겼는데
     * `applyStageMood` 는 안 넘긴 값을 기본으로 되돌린다. 그래서 옥상의 달빛
     * (moonIntensity 0.95)과 푸른 기가 **클라이맥스에서 꺼졌다.**
     * 이제 전부 명시한다 — 서치라이트가 내려온 것처럼 밝아져야 한다.
     */
    arriveMood: {
      fogDensity: 0.015, fogColor: 0x3a5170,
      ambientIntensity: 0.85, ambientColor: 0xa8bcd8, moonIntensity: 1.25,
    },
  },
};
