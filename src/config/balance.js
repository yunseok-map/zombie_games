 ...
 * 방호복(간호사 메시) 재질 보정.
 *
 * Mixamo 원본은 알베도가 거의 순백이라 손전등을 가까이서 받으면 완전히 날아간다.
 * 실제로 공격 사거리(1.9m)에서 찍어 보니 **몸통 주름이 통째로 사라진 흰 덩어리**였다.
 * 3.6m 에서는 멀쩡해서 놓치기 쉬운데, 정작 플레이어가 가장 자주 보는 거리가 1.9m 다.
 * 알베도를 낮추고 거칠기를 올리면 같은 조명에서도 접힘과 주름이 살아난다.
 */
export const ZOMBIE_LOOK = {
  // 알베도 배율. 살짝 누렇게 — 갓 꺼낸 방호복이 아니라 오래 입은 옷으로 보여야 한다
