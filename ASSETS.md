...
### 6-3-G. **새까맣게 구워진 텍스처** — 조명 문제로 착각하기 쉽다 (2026-08-08)

소품 4종의 텍스처가 **전 픽셀 0** 으로 내보내졌다. 게임에서는 손전등을 정면으로
비춰도 검은 실루엣만 보인다.

| 파일 | 죽은 텍스처 |
|---|---|
| `prop_bed.glb` | 3/3장 — **모든 병실의 침대** |
| `prop_bodybag.glb` | 3/3장 |
| `prop_ivdrip.glb` | 9/9장 |
| `prop_standing_body.glb` | 2/2장 |

**증상만 보면 조명 버그처럼 보인다.** 실제로 재질·자기그림자·노멀 뒤집힘을 차례로
의심해 세 번 헛짚었다. 가른 것은 **조명을 아예 안 쓰는 `MeshBasicMaterial` 로
바꿔보는 것**이다 — 그래도 검으면 빛이 아니라 텍스처가 검은 것이다.

파일만 봐도 알 수 있다. **1024×1024 인데 1.9KB** 면 단색 이미지다.

전수 검사 — 이미지 최대값이 0 이면 죽은 것이다:

```
python tools/glb_fix_textures.py --dry-run     # 무엇이 죽었고 무엇으로 채울지
python tools/glb_fix_textures.py               # 원본 모델에서 이식
```

`import_props.py` 를 다시 돌리지 않는다. 24종의 축·크기·원점이 전부 다시 계산돼
한꺼번에 어긋난다. 이 도구는 `glb_shrink_textures.py` 와 같이 **이미지 바이트만**
갈아끼우고 지오메트리·UV·스케일은 건드리지 않는다. 원본과는 **재질 순서 + 역할
(색/노멀/금속거칠기/AO)** 로 짝을 맞춘다.

> 발광(emissive) 맵이 거의 검은 것은 **정상이다** — 빛나는 부분만 밝다.
> 판정 기준은 평균이 아니라 **최대값이 0 인가** 이다.
...
| Zombie Crawl | 〃 |
| Sexy Zombie Girl | 삼각형 **150,000 → 20,000** 데시메이트 · 텍스처 512 WebP 재인코딩 · 키 1.70m 로 정규화 · 원점을 바닥 중앙으로 (`import_props.py`) · **금속도 0.54 → 0** (아래) |
...
> **서 있는 시신은 5,000 삼각형으로는 못 쓴다** (2026-08-08). 처음엔 예산을 아끼려고
> 150,000 → 5,000(97% 감축)으로 구웠는데, 게임 안에서 **사람이 아니라 흩어진 유리 파편**으로
> 보였다. 원본이 머리카락·옷을 **별도 껍질 메시**로 들고 있어서 그만큼 깎으면 껍질끼리
> 뒤엉킨다. **20,000 으로 올리면 실루엣이 살아난다** (1F 정적 삼각형 83k → 98k,
> 예산 350k 안이라 여유가 있다). 파일 0.31MB → 1.03MB.
>
> 그리고 원본 재질의 **금속도가 0.54** 였다. 이 프로젝트의 소품 상한(`SURFACE.propModelMetalMax`
> 0.55)을 그대로 통과해서, 손전등 아래 표면 전체가 흰 점으로 반짝여 젖은 몸이 아니라
> **깨진 셰이딩**으로 읽혔다. 전역 상한을 내리면 침대 프레임·링거대가 같이 죽으므로
> **그 GLB 파일의 `metallicFactor` 만 0 으로** 고쳤다(거칠기 0.9, 금속-거칠기 텍스처 제거).
> **소품을 넣을 때는 삼각형 수만 보지 말고 화면에서 형태가 남는지 확인해라.**
